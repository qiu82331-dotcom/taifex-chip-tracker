...
def compute_all(df):
    ...
    for i in range(len(df)):
        ...
        if op == "👉 隔天進場":
            # 進場價 = 下一列的開盤價
            if i + 1 < len(df):
                entry_price = df.loc[i + 1, "開盤"] if pd.notna(df.loc[i + 1, "開盤"]) else close
            else:
                entry_price = close
            df.loc[i, "未實現損益"] = 0
            ...
            df.loc[i, "未實現損益"] = pnl
            df.loc[i, "最低保證金"] = 71750 + abs(pnl) * 50 if pnl < 0 else 71750
            position = False
        elif position:
            pnl = close - entry_price
            df.loc[i, "未實現損益"] = pnl
...
def write_excel(df, trades_df, val_df):
    ...
    cols1 = ["date", "星期", "開盤", "收盤", "近月", "遠月", "小散戶", "微散戶",
             "結算近減散戶", "主力", "籌碼", "未實現損益", "最低保證金", "信號", "操作"]
    ...
    for ri, (_, row) in enumerate(display.iterrows(), 2):
        ...
        for ci, col in enumerate(cols1, 1):
            ...
            elif col in ("小散戶", "微散戶", "未實現損益", "最低保證金") and (pd.isna(val) if isinstance(val, float) else False):
                val = ""
            ...
            if col == "籌碼" and isinstance(val, (int, float)):
                if val > 100:
                    ...
                    cell.font = red_bold
            if col == "未實現損益" and isinstance(val, (int, float)):
                if val < -300:
                    cell.font = red_bold
                elif val < 0:
                    cell.font = red_font
